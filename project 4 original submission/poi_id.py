...
features_list = ['poi', 'exercised_stock_options',
                 'bonus', 'salary' ] # You will need to use more features
...
def max_finder(data_dict, feature):
    tmp_list = []
    for k in data_dict:
        if data_dict[k][feature] == 'NaN':
            continue
        tmp_list.append(np.float(data_dict[k][feature]))
    return (max(tmp_list))

def rough_scaler(data_dict, feature):
    '''converts value into percentage of highest value and assigns
    to "scaled_"+feature, numpy must be imported'''
    feature_max = max_finder(data_dict, 'bonus')
    for k in data_dict:
        if data_dict[k][feature] == 'NaN':
            data_dict[k]['scaled_' + feature] = 'NaN'
            continue
        data_dict[k]['scaled_' + feature] = ( 
          np.float(data_dict[k][feature]) / feature_max ) 
    pass                        
